...
用户反馈 Consensus 网页有时反复要求登录／授权。本次直接使用当前已加载的 `mcp__consensus__search`，依次执行两次实际调用：

1. `query: "PII-VisBench", page_size: 1`：成功返回一篇论文及摘要。
2. `query: "Defeating Cerberus Privacy-Leakage Mitigation in Vision Language Models", page_size: 1`：成功返回一篇论文；其摘要字段仍缺有效内容，此项与认证状态分开处理。

两次执行均没有重新发起登录或请求人工点击。只读检查发现本机当前使用固定的客户端与连接参数，客户端已持久保存有效凭据及刷新令牌。因此保持现有连接配置，修改技能为直接复用已加载工具、仅在真实认证失败且刷新未恢复时进入单一授权流程；不再将所有账户授权操作一律交给用户。

本次没有复现反复授权，也没有主动注销、清空凭据或制造过期。已验证的是当前连接可无人工点击连续检索；首次授权、过期后刷新以及实际授权按钮自动点击未在本轮重新触发，不能据此声称今后永远无需登录。凭据及账户信息未写入仓库。
