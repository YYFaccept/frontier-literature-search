...
[Consensus 官方 MCP 说明](https://docs.consensus.app/consensus-mcp)提供入口 `https://mcp.consensus.app/mcp`。Consensus 是可选协作工具，不作为技能强制依赖。安装与首次授权只在尚未连接且用户要求接入时执行；已连接时跳过安装和登录命令。不要新增付费订阅或开启额外计费。

### 复用连接与账户授权

- **先调用已有工具**：当前已暴露搜索工具时，直接执行任务所需查询；成功返回即继续，不另做登录或连接检测。工具尚未加载时先用当前环境的工具发现方式查找，不把“未加载”当成“未登录”。
- **保持已授权的客户端**：复用现有连接方式、服务地址、权限范围和客户端管理的持久凭据。不要为一次搜索切换原生连接与转接工具、添加重复连接、注销、清除凭据或重新注册客户端；这些操作可能开启另一套授权会话。令牌刷新交给客户端自身机制处理，不把令牌复制进技能、提示词或仓库。
- **按真实认证状态恢复**：仅在明确提示需要登录、凭据撤销／失效且客户端刷新未恢复时进入认证。普通超时、限流、空结果、摘要缺失或响应解析失败不触发重新登录。恢复后只重试原失败查询一次；同样失败则继续其他来源，并记录未恢复状态。
- **同一连接只保留一个待授权流程**：需要网页登录时复用尚未完成、尚未过期的当前流程，不并行启动多个登录命令或反复生成新链接。等待用户操作期间继续独立检索；用户回复完成后验证原连接，不再要求点击另一条新生成的授权链接。流程已完成或明确失效后不复用旧链接。
- **自动处理已授权范围内的普通操作**：依据当前浏览器可见页面，确认仍是用户已授权的 Consensus 账号、客户端和相同权限范围后，可操作普通“继续／授权”按钮，并遵守当前工具的确认要求。不要把所有 OAuth 页面一律交给用户点击。需要用户输入密码、验证码、选择无法确定的账号，或涉及新增权限时，集中说明所需操作并请求必要协助；同一待处理步骤不重复询问。

首次授权或服务端要求的重新认证仍可能需要用户参与。当前工具能成功检索时，不为验证自动点击能力主动注销或制造授权页面。
...
5. 未连接、认证未完成或工具失败时继续其他来源；明确需要认证时按上面的流程恢复。在检索记录中区分“未使用／调用失败／实际返回结果”；只有真实返回论文并完成相应核验后才能记为 Consensus 实测完成。
