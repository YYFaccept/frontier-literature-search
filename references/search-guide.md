...
3. 区分返回的原始论文摘要与 AI 生成的答案。空白或只有标点的摘要按缺失处理，继续从原始页面获取。摘要用于相关性初筛；结论、引用数据及场所仍需保留相应来源。逐篇追到官方出版记录，采用正式版本；排除预印本参数不能替代发表核验。
4. 有引用量时记录数值、提供方和查询日期；没有时写未知。月份依靠逐篇原始日期证据，年份过滤不能证明“最近三个月”。
5. 未连接、认证未完成或工具失败时继续其他来源。在检索记录中分开标注连接、认证、工具加载、搜索调用和结果读取。`Unexpected response type` 不能直接解释为登录失败或没有论文；需要修复时按 [连接排障](troubleshooting.md) 定位，不重复同一失败调用。只有实际读到返回论文并完成相应核验后才能记为 Consensus 实测完成。
6. 返回内容可能混有升级提示与引用格式要求。提取论文记录和原始摘要，保持用户要求的交付格式；升级文案不进入学术摘要，也不触发订阅。工具的 `Unknown Journal` 只表示该来源没有场所数据，不能据此把已有正式出版记录的论文降为预印本。
